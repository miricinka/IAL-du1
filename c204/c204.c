...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
	...
	char TopChar; //promenna na prvek z vrcholu zasobniku

	while(stackEmpty(s) == 0){ //dokud neni prazdny zasobnik

		stackTop(s, &TopChar); //naplneni TopChar prvkem z vrcholu zasobniku

		if(TopChar == '('){
			stackPop(s);//zavorka se odstrani ale nenahraje do vystupniho pole
			break;//vyskoceni z whilu protoze nalezena '('
		} 

		stackPop(s); //odstraneni prvku
		postExpr[*postLen] = TopChar; //prvek se nahraje do vystupniho pole
		*postLen = ((*postLen) + 1); //inkrementace postlen pro vlozeni dalsiho prvku
	}
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
	...
	while(1){
		...
		//vlozeni pokud je na vrcholu '('
		if(TopChar == '('){
			stackPush(s, c);
			break;
		}
		//na vrcholu je operator s nizsi prioritou
		else if((TopChar == '+' || TopChar == '-') && (c == '*' || c == '/')){
			stackPush(s, c);
			break;
		}
		//jinak
...
char* infix2postfix (const char* infExpr) {
	...
	tStack *s = (tStack*) malloc(sizeof(tStack)); //zasobnik
	if(s == 0){ //osetreni malloc
		return NULL;
	}
	char *postExpr = (char* ) malloc(MAX_LEN *sizeof(char)); //vystupni retezec
	if(postExpr == 0){ //osetreni malloc
		return NULL;
	}

	stackInit(s); //inicializace zasobniku
	...
	while(c != '\0'){
		...
		if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9')){
			//operand se da na vystup
			postExpr[postLen] = c;
			postLen = postLen + 1;
		}
		else if(c == '('){ //c je leva zavorka
			stackPush(s, c);	

		}
		else if(c == ')'){ //c je prava zavorka
			untilLeftPar(s, postExpr, &postLen);
		}
		else if(c == '+' || c == '-' || c == '*' || c == '/'){ //operator
			doOperation ( s, c, postExpr, &postLen ); //zpracovani operatoru
		}
		else if(c == '='){
		
			while(stackEmpty(s) == 0){ //dokud neni zasobnik prazdny
				stackTop(s, &TopChar); 
				postExpr[postLen] = TopChar;
				postLen = postLen + 1;
				stackPop(s);
			}
			postExpr[postLen] = '='; //umisteni = na konec stringu
			postLen = postLen + 1;
		}
		c = *(++infExpr);
	}
	postExpr[postLen] = '\0';	//prazdny znak na konec stringu
	free(s); //uvolneni pameti
  	return postExpr;   
