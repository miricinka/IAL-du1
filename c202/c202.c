...
*/
	...
	if(s != NULL) {
		s->top = -1; //inicializace zasobniku = zasobnik je prazdny top -1
	}
	else{ //parametr s je NULL, volani chybove funkce
...
*/
	return !(1 + s->top); //kdyz top == -1, returne cislo ruzne od nuly, jinak returne 0
...
*/
	...
	return !(s->top - (STACK_SIZE - 1)); // stack_size -1 protoze se uklada od nuly. 
}
...
*/
	...
	if(stackEmpty(s)){ //kontrola zda je zasobnik prazdny, vraci error
		stackError(SERR_TOP);
		return;
	}

	*c = s->arr[s->top]; //predani hodnoty z vrcholu zasobniku parametru c
}
...
*/
	...
	if (stackEmpty(s) == 0){ //kontrola zda je zasobnik prazdny
		s->top = s->top - 1; //odstraneni prvku - 
...
*/
	...
	if(stackFull(s) != 0){ //kontrola zda jiz neni zasobnik plny
		stackError(SERR_PUSH);
	}
	else { //muzem vkladat
		s->top = s->top + 1; //posunuti ukazatele na vrchol zasobniku o jeden vys
		s->arr[s->top] = c; //nova hodnota se ulozi do nove pripraveneho policka
	}
}
